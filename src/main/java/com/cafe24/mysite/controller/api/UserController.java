...
//	@ApiImplicitParams({
//		@ApiImplicitParam(name = "email", value = "이메일주소", required = true, defaultValue = "")		// or path
//	})
	...
	public ResponseEntity<JSONResult> join(
		...
		if(result.hasFieldErrors()) {
			...
			for(ObjectError err : list) {
				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(JSONResult.fail(err.getDefaultMessage()));
			}
	...
	public ResponseEntity<JSONResult> login(
		...
		if(!validatorResults.isEmpty()) {
			//for(ConstraintViolation<UserVo> validatorResult : validatorResults) {
				...
				String message = messageSource.getMessage("Email.userVo.email", null, null);
